...
import TabsA11y from "../../../components/TabsA11y";
import { submitStage, StageName } from "../../../lib/api-stages";
...
export default function StageTabs({ recordNo, onSubmitted }: Props) {
  ...
  return (
    <section aria-label="Work order stages" className="w-full">
      <TabsA11y
        tabs={stages.map((s) => s.name)}
        activeIndex={active}
        onChange={setActive}
        panels={stages.map((s) => ({
          key: s.name,
          node: s.element(recordNo, (data) => handleSubmit(s.name, data)),
        }))}
      />
    </section>
  );
