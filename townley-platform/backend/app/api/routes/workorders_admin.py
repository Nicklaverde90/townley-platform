...
def export_csv(db: Session = Depends(get_db), _user=Depends(require_admin)):
    ...
    for r in rows:
        writer.writerow([r.RecordNo, r.Status or "", r.Description or "", r.CreatedAt or ""])
...
def hard_delete(record_no: int, db: Session = Depends(get_db), user=Depends(require_admin)):
